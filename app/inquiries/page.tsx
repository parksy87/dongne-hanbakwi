...
import { useUserInquiries, useDeleteInquiry } from "@/hooks/useInquiries";
...
export default function InquiriesPage() {
  ...
  useEffect(() => {
    ...
    const markRead = async () => {
      ...
      await updateUserPreferences(user.uid, { lastSeenInquiryAnswerAt: now });
      setUser({ ...user, lastSeenInquiryAnswerAt: now });
      queryClient.invalidateQueries({ queryKey: ["inquiries", "unread"] });
    };
    markRead();
  }, [user?.uid]);

  const handleDelete = async (inquiry: { id: string }) => {
    if (!user) return;
      ...
      <div className="page-container">
        ...
        {isLoading ? (
          ...
          <InquiryList
            ...
            deletePending={deleteInquiry.isPending}
          />
        )}
      </div>
    </AuthGuard>
